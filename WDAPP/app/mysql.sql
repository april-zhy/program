...
select date_format(a.endTime,'%Y-%m-%d %H:%m:%s') from a (select * from tb_action);

select *,date_format(endTime,'%Y-%m-%d %H:%m:%s') from tb_action;

select date_format(endTime,'%Y-%m-%d %H:%m:%s')as startTime,date_format(endTime,'%Y-%m-%d %H:%m:%s') as endTime from tb_action;
...
select * from (g.*,(g.normalPrice*t.actionAgion*0.1) as realPrice from tb_goods g left join (select b.*,a.goodsId from tb_actioninfo a,(select * from tb_action where startTime<=now() and endTime>=now()) b where a.actionId=b.actionid) t on t.goodsId=g.goodsId);



select * from (select g.*,(g.normalPrice*t.actionAgion*0.1) as realPrice from tb_goods g left join (select b.*,a.goodsId from tb_actioninfo a,(select * from tb_action where startTime<=now() and endTime>=now()) b where a.actionId=b.actionid) t on t.goodsId=g.goodsId) tb where tb.goodsId=110055;
